namespace Il2Cpp {
    ...
    function unityEngineCall(method: string): string | null {
        const handle = api.resolveInternalCall(Memory.allocUtf8String("UnityEngine.Application::" + method));
        const nativeFunction = new NativeFunction(handle, "pointer", []);

        return handle.isNull() ? null : new String(nativeFunction()).asNullable()?.content ?? null;
    }
}
